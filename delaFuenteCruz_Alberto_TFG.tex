...
\newline \newline
Este análisis no se limita únicamente a la medición del tráfico, si no que puede utilizarse como una herramienta de negocio mediante la cual mejorar el \textit{marketing} y el impacto de una web, además de aportar la información necesaria para realizar campañas publicitarias por zonas geográficas, variables demográficas, etc. Se trata de conocer al usuario y comprender las tareas que realiza dentro de un sitio a través de su \textit{tracking}. Este \textit{trackeo} puede realizarse de distintas formas, veamos algunas a continuación.  
...
\newline \newline
Gracias a estas organizaciones es posible saber quien es el dueño de cada dirección, conociendo además detalles de la conexión \cite{ripe}. 
\subsection{Click analytics}
lorem %https://en.wikipedia.org/wiki/Web_analytics ipsum
\subsection{Customer lifecycle analytics}
lorem ipsum 
\subsection{Problems with cookies}
lorem ipsum 
