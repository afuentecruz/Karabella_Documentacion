...
\end{figure}
%%% ------
Siendo los casos de uso desde el punto de vista de la captura de información los siguientes.
\newline
\newline
...
\newline 
\newline
Por lo tanto, si prestamos atención a las necesidades del sistema entendemos que será necesaria la siguiente arquitectura. 
%%% ------ 
\begin{figure}[H]
	\begin{center}
		\includegraphics[scale=0.45]{pictures/architecture/ArquitecturaBasica01.png} % include ./img/imagen.[pdf|png|jgp] si es pdflatex o ./img/imagen.eps si es latex
	\end{center}
	\caption[Arquitectura general]{Arquitectura general del sistema.}
\end{figure}
%%% ------
Donde tenemos tres capas intermedias entre la interfaz de usuario y el almacenamiento. Estas tres capas se encargarán de la lógica de negocio del sistema. Veamos estas cinco capas. 
\begin{itemize}
  \item UI. La interfaz del usuario de nuestra aplicación. En ella desarrollará el uso del teléfono interactuando con el. Esta es por tanto la principal fuente de datos de nuestro proyecto. 
  \item Capturador de información. Primera capa de nuestra lógica de negocio. Será la encargada de capturar la información directamente de la UI y suministrarla en bruto a las capas inferiores, encargadas ellas de procesarlas. Por lo tanto su función es, sencillamente, capturar la información y pasarla hacia abajo. Esto es así con el fin de desarrollar una lógica lo más mínima posible, de modo que se libere de carga computacional innecesaria al proceso, dado la posibilidad de recibir cargas descomunales de información en momentos puntuales.  
  \item Analizador sintáctico. Es la capa encargada de realizar un primer análisis a los datos para ordenarlos de acuerdo a unas reglas. Esta ordenación no tiene más propósito que analizar de acuerdo a unas reglas los datos, para o bien encapsularlos en unidades de las que se conoce su forma, o descartarla en caso de que no satisfaga estas reglas. 
  \item Analizador semántico. Se encargará de interpretar los datos suministrados por la capa sintáctica. Dotará por tanto de un sentido a los datos bien formados y estructurados de la capa superior. Esto se realizará desarrollando sendas máquinas de estado que permitan situar los datos en su contexto y extraer así su significado. 
  \item La capa de persistencia. Nos permitirá almacenar los datos en local, disponibles para su consulta. 
\end{itemize}
De esta forma obtenemos una arquitectura inicial que nos plantea una separación de funcionalidades en capas lo suficientemente atómicas como para poder realizar una arquitectura sencilla y eficaz basada en este planteamiento inicial. 
\chapter{Diseño}
